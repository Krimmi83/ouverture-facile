...
(async function(){
  ...
  importLabel.addEventListener('click', e=>{ if(!isAdmin()){ e.preventDefault(); alert('Admin required.'); } });

  function setSortOptions(){ const opts=sortsByGame[currentGame]||[]; sortSel.innerHTML=opts.map(([v,l])=>`<option value="${v}">${l}</option>`).join(''); }
  ...
  async function render(){
    ...
    if(currentGame==='smash'||currentGame==='mariokart'){
      headRow.innerHTML=`<th>#</th><th>Username</th><th>Wins</th><th>Losses</th><th>Win rate</th>`;
      tbody.innerHTML=list.map((p,i)=>{
        ...
        return `<tr><td>${i+1}</td><td>${escapeHtml(p.username)}</td>
          ...
          <td><span class="pill rate">${fmtPercent(winRate(g))}</span></td></tr>`;
      }).join('');
    }else if(currentGame==='demineur'){
      headRow.innerHTML=`<th>#</th><th>Username</th><th>Difficulty</th><th>Score</th>`;
      tbody.innerHTML=list.map((p,i)=>{
        const g=p.games?.demineur||{};
        return `<tr><td>${i+1}</td><td>${escapeHtml(p.username)}</td><td>${escapeHtml(g.difficulty||'-')}</td><td>${g.score|0}</td></tr>`;
      }).join('');
    }else if(currentGame==='minecraft'){
      headRow.innerHTML=`<th>#</th><th>Username</th><th>Kills</th><th>Deaths</th><th>K/D</th>`;
      tbody.innerHTML=list.map((p,i)=>{
        const g=p.games?.minecraft||{}; const kdr=kdRatio(g);
        return `<tr><td>${i+1}</td><td>${escapeHtml(p.username)}</td>
          <td><span class="pill">${g.kills|0}</span></td>
          <td><span class="pill">${g.deaths|0}</span></td>
          <td><span class="pill kd">${(isFinite(kdr)?kdr.toFixed(2):'-')}</span></td></tr>`;
      }).join('');
    }
  }
