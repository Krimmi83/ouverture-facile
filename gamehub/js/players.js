...
(async function(){
  ...
  function setAdminUI(){
    ...
    for(const el of document.querySelectorAll('[data-admin-only]')) el.style.display = isAdmin()? '' : 'none';
    for(const el of form.querySelectorAll('input,select,button')){ if(el===resetBtn) continue; el.disabled=!isAdmin(); }
  }
  ...
  tabs.addEventListener('click', e=>{
    ...
    for(const panel of document.querySelectorAll('.game-panel')) panel.style.display=(panel.dataset.game===currentGame)?'':'';
    render();
  });
  ...
  addDemoBtn.addEventListener('click', async ()=>{
    try{
      ...
      const demo={ id:nextId, username:`Player${nextId}`, games:{
  ...
  clearAllBtn.addEventListener('click', async ()=>{
    ...
    localStorage.setItem('gamehub_users_v3','[]'); await render();
  });
    ...
    try{
      ...
      if('showDirectoryPicker' in window){
        try{
          ...
          const fh=await dir.getFileHandle(fname,{create:true}); const w=await fh.createWritable();
          await w.write(text); await w.close(); alert(`Sauvegarde créée dans “joueurs” : ${fname}`); return;
      ...
      alert('Fichier de sauvegarde téléchargé.');
  ...
  window.editPlayer = async (id)=>{
    ...
    fId.value=p.id; fUser.value=p.username;
    const g=p.games||{};
  ...
  window.deletePlayer = async (id)=>{ try{ await api.deletePlayer(id); await render(); }catch(e){ alert('Admin required.'); } };
  ...
  saveBtn.addEventListener('click', async ()=>{
    ...
    const payload={ id, username, games:{
    ...
    try{ await api.upsertPlayer(payload); form.reset(); await render(); }catch(e){ alert('Admin required.'); }
  });

  resetBtn.addEventListener('click', ()=>{
    fId.value=''; fUser.value='';
  ...
  async function render(){
    ...
    const list=(await api.listPlayers()).filter(p=>!q||p.username.toLowerCase().includes(q));

    let rows='';
    if(currentGame==='smash'||currentGame==='mariokart'){
      headRow.innerHTML=`<th>Id</th><th>Username</th><th>${currentGame==='smash'?'Smash':'Mario Kart'} (W-L)</th><th>Actions</th>`;
      rows=list.map(p=>{
      ...
      headRow.innerHTML=`<th>Id</th><th>Username</th><th>Difficulté</th><th>Score</th><th>Actions</th>`;
      rows=list.map(p=>{
      ...
      headRow.innerHTML=`<th>Id</th><th>Username</th><th>K/D</th><th>Actions</th>`;
      rows=list.map(p=>{
